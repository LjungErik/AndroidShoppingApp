...
class UpdateShoppingListItem(transformer: Transformer<Boolean>,
    ...
    override fun createObservable(data: Map<String, Any>?): Observable<Boolean> {
        ...
        shoppingListItemEntity?.let {
            return Observable.fromCallable {
                ...
                shoppingListItemRepository.updateShoppingItem(entity)
                return@fromCallable true
            }
        }?: return Observable.error({IllegalArgumentException("ShoppingListItemEntity must be provided")})
    }

    fun update(shoppingListItemEntity: ShoppingListItemEntity): Observable<Boolean> {
